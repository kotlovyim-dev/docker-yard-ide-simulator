...
import type { LessonContext, EngineContext, Lesson, Objective } from "../engine/types";

export type SelectLessonEvent = { type: "SELECT_LESSON"; lessonId: string };
export type CheckObjectivesEvent = { type: "CHECK_OBJECTIVES"; engineSnapshot: EngineContext };
...
function currentObjective(ctx: LessonContext): (Objective & { predicate?: (c: EngineContext) => boolean }) | undefined {
    if (!ctx.currentLessonId) return undefined;
    const lesson = findLesson(ctx, ctx.currentLessonId);
    return lesson?.objectives[ctx.currentObjectiveIndex] as (Objective & { predicate?: (c: EngineContext) => boolean }) | undefined;
}
...
export const lessonMachine = setup({
    types: {
        ...
        input: {} as { availableLessons?: Lesson[] },
    },

    actors: {
        loadLesson: fromPromise(async ({
            input,
        }: {
            input: { ctx: LessonContext; lessonId: string };
        }): Promise<{ lesson: Lesson }> => {
            const lesson = input.ctx.availableLessons.find((l) => l.id === input.lessonId);
    ...
    states: {
        ...
        loading: {
            invoke: {
                ...
                input: ({ context, event }) => ({
                    ctx: context,
                    lessonId: (event as SelectLessonEvent).lessonId ?? context.currentLessonId,
                }),
                onDone: {
                    target: "active",
                    actions: assign(({ event }) => {
                        ...
                        return {
                            ...
                            currentObjectiveIndex: 0,
                            scenarioSnapshot: {
                                images: Object.fromEntries(lesson.initialImages.map((img) => [img.id, img])),
                                containers: Object.fromEntries(lesson.initialContainers.map((c) => [c.id, c])),
                                files: Object.fromEntries(lesson.initialWorkspace.map((f) => [f.path, f])),
                            },
                        };
                    }),
                },
            ...
            on: {
                CHECK_OBJECTIVES: {
                    actions: [
                        ...
                        raise(({ context, event }) => {
                            const obj = currentObjective(context);
                            if (!obj?.predicate) return { type: "CONTINUE" as const };
                            const passed = obj.predicate(event.engineSnapshot);
                            if (passed) return { type: "OBJECTIVE_COMPLETE" as const };
                ...
                OBJECTIVE_COMPLETE: {
                    actions: assign(({ context }) => {
                        const lesson = findLesson(context, context.currentLessonId ?? "");
            ...
            always: [
                {
                    guard: ({ context }) => {
                        const lesson = findLesson(context, context.currentLessonId ?? "");
                        return context.currentObjectiveIndex >= (lesson?.objectives.length ?? 0);
                    },
                    target: "celebrating",
                },
                { target: "active" },
        ...
        celebrating: {
            entry: assign(({ context }) => ({
                completedLessonIds: context.currentLessonId
                    ? [...new Set([...context.completedLessonIds, context.currentLessonId])]
